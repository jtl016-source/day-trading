...
**2026-07-05 — SdkProbe v1 findings (ground truth from real MW install) + getBars switch + contract→root symbol fold**
- **forEachBar is UNUSABLE via reflection**: `Instrument.forEachBar(long,long,BarSize,boolean,BarOperation)` exists, but building a `java.lang.reflect.Proxy` for the obfuscated `BarOperation` interface throws `IllegalArgumentException: methods with same signature b() but incompatible return types`. The proxy-callback approach (`tryForEachBar` in LiveBarRelay, `probeForEachBar` in SdkProbe) is dead — DELETED both.
- **getBars is the replacement**: verified `List getBars(long fromMs, long toMs, BarSize, boolean rth)` and `List getBars(long endMs, int count, BarSize, boolean rth)` (ignore the extra-`d`-param overloads). Plain `List` return, no callback interface. LiveBarRelay now backfills via `tryGetBars` — locates `getBars(long,long,<BarSize>,boolean)` once (cached Method), invokes with rth=false, extracts each element via cached accessor Methods resolved from the first element's class (getStartTime→getTime, getOpen/High/Low/Close, getVolume→getVolumeAsFloat). Same validity rules (skip h<l, o<=0, bad ts). Auto-degrade to the DataSeries slice (`source:"chart"`) is unchanged and also fires when `barSizeObj` is null. SdkProbe v2 (header bumped to `=== SDK PROBE v2 ===`) now calls getBars for the 24h / 2-years-ago / 6-months-ago windows and dumps the first element's class + method list.
- **BarSize accessors verified**: `getInterval()=5`, `getIntervalMinutes()=5`, `getIntervalSeconds()=300` for a 5-min chart; `DataSeries.getBarSize()` exists; `DataSeries.getStartTime(i)` returns epoch **milliseconds** aligned to the bar. `resolveViaBarSize` simplified to try getIntervalMinutes → getIntervalSeconds/60 → getInterval (all treated as minutes); the seconds-guessing in `mapBarSizeValue` is gone. Unknown minutes now pass through as `String.valueOf(minutes)` (server parses arbitrary "N" as N minutes) instead of being force-bucketed.
- **getSymbol() returns the CONTRACT code, not a continuous symbol** — e.g. `MESU6`, not `MES`. So MW-ingested history was keying DB rows per-contract (fragmenting every quarterly roll) while the client only ever queries by root (`MES`/`ES`). Fix on the SERVER side (no Java symbol change):
  - `shared/symbols.ts` `normalizeSymbol` now folds a contract code to its root via regex `^(MES|ES|MNQ|NQ|MYM|YM|M2K|RTY|MCL|CL|MGC|GC|SIL|SI|NG|ZB|ZN)([FGHJKMNQUVXZ])(\d{1,2})$` → root capture. **Longer roots MUST precede their prefixes in the alternation** (MES before ES, MNQ before NQ, etc.) or micros mis-fold.
  - `server/db.ts` `foldContractSymbols()` (exported, runs once at startup, idempotent): for each `cached_candles`/`signal_history` symbol where `normalizeSymbol(s)!==s`, `INSERT OR IGNORE` into the root bucket (existing root rows win collisions — they're what the client reads) then `DELETE` the contract rows, all in one transaction. Correct moved/dropped accounting = `INSERT.changes` (landed) vs `DELETE.changes - moved` (collisions IGNOREd).
  - `server/routes.ts`: `/api/data/cached-continuous/:symbol/:interval` and `/api/data/gaps/:symbol/:resolution` now run the param through `normalizeSymbol` instead of `.toUpperCase()`, so a client (or MW) querying `MESU6` still hits the `MES` bucket.
- Verification: `npm run check` clean; both studies stub-compile (`javac --release 17`, exit 0); migration smoke-test asserts all rows fold to `MES`, the colliding row keeps the original root values, and counts are moved=1/dropped=1 for the contract with one unique + one colliding row.
- Confidence: high
