...
**2026-08-06 — Taught by User: NEVER present a replay as "what the program did"**
- **The failure**: the entire candle-by-candle monitor + EOD report below was a Yahoo MES=F replay of `computeOptimizedSignals` presented as program behavior. The REAL program (queried afterward via the user's tunnel at `/api/signals/history/MES/{5m,15m}`) fired **3 signals today, all 15m, all `signalType:"fact-engine"`** — a Long 09:15 bucket +11.25 (FG+Vec), Short 11:30 +5.49 ×2 contracts (FG+Fr+YB), Short 12:30 +2.99 ×2 (Fr+YB) — versus the replay's fictional 18 signals. TWO independent root causes: (1) Yahoo bars ≠ MotiveWave bars, so candle-derived zones reshuffle; (2) **the running app's engine is not in the GitHub repo at all** — no branch contains the fact-engine (facts arrays, comboKey, suggestedContracts, riskFlags, anchor). The repo lags the deployed program by an entire strategy rewrite.
  - Action: before ANY claim about live behavior, fetch `signal_history` from the running app (each row's `label` + `confirmations.facts` is the program's own stated reasoning — use it). Use the app's `cached_candles` (`/api/data/cached-continuous/ES/5m`) for bar context. Yahoo+repo replays must always be labeled "replay estimate (repo code, Yahoo data)" and never summarized as program actions. If the app is unreachable, say so and ask — do not silently substitute a replay.
  - Action: check whether the repo matches the running app before analyzing "the strategy" (git grep for markers the DB rows carry, e.g. `fact-engine`/`comboKey`). If absent, the repo is stale — ask the user to push the running code.
  - Confidence: high (bar-verified against the program's own DB).

**2026-08-06 (EOD) — [REPLAY ESTIMATE — see correction above: NOT what the program did] Candle-by-candle monitor + session post-mortem**
- **New tool: `scripts/candle-explain.ts`** — per-bar gate narrator for THE program strategy. Re-walks the exact `computeEngineSignals` gate sequence under `OPTIMIZED_GATES` (zone test-and-hold with named zones, body, per-direction cooldown, HOD suppression, 60m veto) for both intervals and cross-checks fired signals against `computeOptimizedSignals` (0 mismatches across the full day). Modes: latest bar / `--day` / `--since=<ts>`. Same Yahoo MES=F source and optimistic TP-before-SL caveat as replay-day.ts.
- **The day (Yahoo bars, optimistic bound): 18 signals, 12W/6L (66.7%), +40 pts** (15m component +24, 5m +16). Morning trend-down leg then an afternoon range ~7727–7749. Support-buy longs in the range went 5-for-6 in the afternoon; the engine's mean-reversion behavior that lost on 08-05's box WON today because the box held.
- **NEW BUG-CLASS FINDING — doji fires BOTH directions on the same bar**: 14:05 ET 5m bar O 7738.5 C 7738.5 (exact doji) passed `close >= open` (long body) AND `close <= open` (short body), tested a support and a resistance zone simultaneously, and fired a Long AND a Short on the same bar — a self-hedge that resolved +4/−4 = 0 minus costs on the 14:10 bar (H 7742.5 hit long TP1 and short SL together). Candidate fix: strict inequalities (or skip doji bars) in the body gate — validate on the benchmark month via the optimizer first.
- **Known failure mode repeated — wrong-edge range shorts**: both 15m losses were shorts entered near the BOTTOM of the afternoon box (13:00 @ 7731, 15:30 @ 7733; range floor ~7727) and stopped on the bounce, exactly the 08-05 "alternates long/short inside a range" defect. The proposed range/chop filter would have saved −8 pts today while none of the day's winners entered at a wrong edge.
- **Same-bar TP/SL ambiguity again**: the 15:45 5m long (entry 7734.25, TP1 7738.25, SL 7730.25) was credited a win from the 15:50 bar spanning BOTH (H 7738.25 / L 7727). Flip it and the day is +32, not +40.
- **Cooldown earned its keep at 13:45**: a fully-valid 5m short setup (resistance test + bearish close) was blocked with 1 cooldown bar left; it would have stopped out on the 13:50 bounce (SL 7734.75, 13:50 H 7734.75). Logged as evidence FOR the 10-bar cooldown in ranges.
- **Ops lesson (remote monitoring)**: session-only cron jobs die with the remote container — the live loop stalled 14:05–16:05 ET when the container was reclaimed and bars arrived as one catch-up batch. Recurring monitoring needs a durable Routine (create_trigger), not CronCreate.
- Confidence: high for the bar-verified observations; low for the doji/chop candidate fixes until backtested.

**2026-08-06 — "No signals on the chart" + "no 15m trades": three compounding bugs fixed**
- **Signal markers silently vanish on coarser charts** — CandlestickChart's signal loop did `timeToCoordinate(sig.time)` and `continue`d on null. A 5m/15m-tagged signal whose bar time doesn't exist on the displayed time scale (2/3 of 5m signals on the 15m chart; ~all signals on the 60m chart — none of today's 9 signals were hour-aligned) drew NOTHING. Since the viewed `interval` is persisted in localStorage, a user last on the 60m chart sees an empty chart every session. Fix: snap unmatched signal times to the displayed bar CONTAINING them (binary search greatest bar time ≤ sig.time, accept only within one displayed-interval; interval detected min-delta-from-`Infinity` per the gap-detection rule).
- **60m chart fed 15m bars to the 5m strategy component** — the bg15m query aggregates `agg5mTo15m()` INSIDE `queryFn`, but `allConfluenceSignals` used `bg15mData.candles` as the 5m source (the comment claimed it was "5m-resolution" — it wasn't). The 5m component on the 60m chart computed wrong, mislabeled signals. Fix: bg15m query now returns `{ candles: 15m, raw5m: 5m }`; the memo uses `raw5m` (falling back to `candles` for stale cache shape).
- **Pre-2026-07-17 persisted `autoTradeIntervals: ["5m"]` blocks all 15m orders forever** — the 7/17 note said "persisted values survive, so existing users keep their choice", but that "choice" was just the old DEFAULT, and the only symptom is an 8-second toast. Today it cost both completed 15m signals (each +8 winners). Fix: one-time migration — if `autoTradeIntervalsV2` flag absent from mwb_settings, re-seed to ["5m","15m"]; flag written on every persist so post-migration choices stick. Lesson: when a persisted setting's DEFAULT changes meaning, migrate with a version flag — never assume an old persisted default is a user decision.
- Diagnosis method: `scripts/replay-day.ts 2026-08-06` proved the engine DOES emit 15m signals today (09:30 L, 10:15 S — both +8 wins; 12:15 L open), so the absence had to be client-side filtering/rendering, which narrowed the search to the marker loop and the interval gate.

**2026-08-05 — Trade post-mortem: why today's trades fired and why the afternoon ones failed**
- **How this was reconstructed**: no DB access in this session, so the day was replayed with `scripts/replay-day.ts` (new, kept for future post-mortems): fetches 10 days of MES=F 5m bars from Yahoo and runs `computeOptimizedSignals` — the exact auto-trader code path. Caveats: Yahoo bars may differ slightly from the MotiveWave feed, and `btWalkForward` checks TP before SL within a bar, so the replay is the OPTIMISTIC bound.
- **The day**: RTH O 7809.5 / H 7820.25 / L 7745.75 / C ~7748 — a −60 pt trend-down day with a midday consolidation box (~7762–7774, 12:00–15:50 ET) and a 20-pt flush at 15:55 ET.
- **What fired**: 19 signals (14×5m, 5×15m), ALL tier "safe" — every one was the standard rule (RTH candle tests-and-holds an ICT zone ±2 pts + closes directionally). Replay: 10 wins / 9 losses / +28 pts optimistic. Morning trend phase (09:30–12:00): 6-for-6 wins. Midday chop + afternoon (12:15–15:50): 3-for-12, including 6 straight losses 12:15–14:00.
- **Root causes of the failures (each verified against the bar data):**
  1. **No chop filter — the strategy alternates long/short inside a range.** In the 12-pt midday box, zone edges kept "testing and holding" in both directions: long 12:15 @7772.25 (range TOP, TP1 7780.25 required a breakout that never came), short 12:35 @7766.5 and 13:25 @7764.75 (near range BOTTOM), long 14:00 @7773 (range top again). The 10-bar cooldown is per-direction, so opposing signals legally alternated every 2–5 bars. 9 losses, nearly all at the wrong edge of the box. This is the same defect the user taught on 2026-04-19/04-10 ("resistance within TP1 range") — the engine only checks day-HOD proximity, never local range structure.
  2. **The 60m declining-vector long veto is structurally blind to intraday trend days.** Verified by dumping the 60m vector: it sat FROZEN at 7761.25 from 10:00 ET through the close (`declining=false` on every bar) while price fell 60 pts, because `Highest(Lowest(low,20),20)` on 60m bars needs ~40 hours of lower lows before it ticks down. Result: 6 of the 9 losses were LONGS taken against a −40-to−60-pt day that the veto was designed to prevent. The veto only catches multi-DAY declines, not "today is a down day."
  3. **Same-bar TP/SL ambiguity inflates the replayed wins.** 3 of the 10 wins (09:30 long, 10:20 long, 10:50 short) had a next bar whose range spanned BOTH TP1 and SL (e.g. 09:35 bar H 7820.25 / L 7804.5 vs TP1 7818 / SL 7810). Walk-forward credits TP first; live fills could have taken the SL first. Flipping those three turns +28 into ~+4 — with slippage, roughly flat-to-negative. If the live day felt like it "failed", this plus cause 1 is why.
  4. **Dual-interval double exposure**: 15:00 ET fired the 5m AND 15m long on the same bar read; both stopped out (−8 pts on one wrong idea). The intervals share no exposure limit.
  5. **No late-session entry cutoff**: the 15:50 long entered 5 minutes before the 15:55 20-pt flush. Session-settle EXITS exist, but entries are allowed until the bell.
- **Candidate fixes — DO NOT ship on one day's data (2026-07-16 lesson: validate on the benchmark month via the optimizer first):**
  - Down-day long suppression: skip longs when price is > ~20 pts below the RTH open. Today that blocks all 6 losing longs and sacrifices only the 12:20 win (+8) → day goes from +28 to +48 optimistic. Mirror-image short suppression on up days should be tested at the same time.
  - Range-compression filter: skip entries when the last N bars' total range < threshold or price keeps crossing the 20-bar midpoint (chop detection) — targets cause 1 directly.
  - Faster intraday trend gate to replace/augment the 60m vector veto (e.g. 60m close slope, or "price below flat 60m vector"), since the vector itself cannot see same-day trends.
  - One-position-per-direction cap across intervals (cause 4) and a no-new-entries cutoff in the final 15–20 min of RTH (cause 5).
  - Confidence: medium for the diagnosis (bar-verified), low for each fix until backtested.
