...
- Deployment context: `railway/fix-deploy-3668ac` branch exists — Railway is the deploy target for the Express server; MotiveWave TickRelay must also point at the deployed `/ws/mw-feed` for live ticks to reach a remote phone.
- **PWA implemented as the no-computer path** (user was away from their computer, so Expo/EAS steps were impossible — the app code and the Expo account both live there). The web client is now installable: `manifest.webmanifest` + 192/512/maskable/apple-touch icons (generated by a zero-dependency PNG encoder script — no ImageMagick/sharp in this env; icons drawn programmatically: yellow box + teal/red candlesticks on #131722), `sw.js` (network-first navigations, cache-first hashed assets, NEVER caches `/api/` or `/ws/` — live bars must always be network), PWA meta in `index.html`, SW registration in `main.tsx` gated on `import.meta.env.PROD`. Vite copies `client/public/*` to `dist/public/` root automatically — verified in the build. "Add to Home Screen" on the deployed URL gives a real app icon with no Expo Go.
