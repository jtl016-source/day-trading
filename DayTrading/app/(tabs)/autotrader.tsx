...
import { GlowPulse } from '@/components/animated-ui';
...
export default function AutoTraderScreen() {
  ...
  function startHold() {
    ...
    holdAnimRef.current = Animated.timing(holdProgress, { toValue: 1, duration: HOLD_MS, useNativeDriver: false });
    holdAnimRef.current.start(({ finished }) => {
      holdProgress.setValue(0);
      if (finished) {
        const next = !enabled;
        Haptics.notificationAsync(next ? Haptics.NotificationFeedbackType.Warning : Haptics.NotificationFeedbackType.Success).catch(() => {});
        setIsSyncing(true);
        fetch(`${apiBaseUrl}/api/trade/settings`, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ enabled: next }),
        }).then(() => setEnabled(next)).catch(() => {}).finally(() => setIsSyncing(false));
      }
    });
  }
  ...
  const resetLock = useCallback(async () => {
    ...
    try {
      ...
      const d = await r.json();
      Alert.alert(d.ok ? 'Lock Reset' : 'Not Connected',
  ...
  return (
    <SafeAreaView style={s.safe} edges={['top']}>
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <GlowPulse color={armColor} active={armState === 'LIVE'} radius={22} style={s.armBannerGlow}>
          <View style={[s.armBanner, { borderColor: armColor + '60', backgroundColor: armColor + '12' }]}>
            <View style={s.armStateRow}>
              ...
              <Text style={[s.armStateLabel, { color: armColor }]}>
                {armState === 'LIVE' ? 'LIVE' : armState === 'ARMED' ? 'ARMED' : 'DISARMED'}
              </Text>
              {armState !== 'OFF' && (
                <Text style={s.armSubLabel}>
                  {armState === 'LIVE' ? 'Trade open · monitoring'
                    : `${contractType} · ${contracts} contract${contracts > 1 ? 's' : ''}`}
                </Text>
              )}
            </View>
            {armState !== 'OFF' && (
            ...
            <View style={s.holdOuter}>
              ...
              <Pressable style={s.holdBtn} onPressIn={startHold} onPressOut={cancelHold} disabled={isSyncing || !connected}>
                {isSyncing ? <ActivityIndicator color={armColor} size="small" /> : (
        ...
        <Section label="CONTRACT">
          <View style={s.chipRow}>
            {(['MES', 'ES'] as ContractType[]).map(t => (
              <Pressable key={t} onPress={() => setContractType(t)}
                style={[s.chip, contractType === t && { borderColor: Trading.accent, backgroundColor: Trading.accent + '18' }]}>
        ...
        <Section label="EXIT MODE">
          <View style={s.chipRow}>
            {([false, true] as const).map(tp1 => (
              <Pressable key={String(tp1)} onPress={() => setTp1Only(tp1)}
                style={[s.chip, tp1Only === tp1 && { borderColor: Trading.accent, backgroundColor: Trading.accent + '18' }]}>
          ...
          <View style={s.chipRow}>
            {(['both', 'long', 'short'] as Direction[]).map(d => (
              <Pressable key={d} onPress={() => setDirection(d)}
                style={[s.chip, direction === d && {
                  borderColor: d === 'long' ? Trading.long : d === 'short' ? Trading.short : Trading.accent,
                  backgroundColor: (d === 'long' ? Trading.long : d === 'short' ? Trading.short : Trading.accent) + '18',
                }]}>
...
const s = StyleSheet.create({
  ...
  armBannerGlow: { margin: 14, borderRadius: 12 },
  armBanner: { borderRadius: 12, borderWidth: 1, padding: 18, gap: 12 },
  ...
  armPulse: { width: 10, height: 10, borderRadius: 5, shadowOffset: { width: 0, height: 0 }, shadowRadius: 6, shadowOpacity: 0.9, elevation: 4 },
  staleBanner: { backgroundColor: Trading.riskiest + '22', borderRadius: 6, padding: 8, borderWidth: 1, borderColor: Trading.riskiest + '60' },
  staleText: { color: Trading.riskiest, fontSize: 11, fontWeight: '700', textAlign: 'center', letterSpacing: 0.5 },
  holdOuter: { borderRadius: 10, overflow: 'hidden', borderWidth: 1, borderColor: Trading.border, minHeight: 52 },
  holdBtn: { minHeight: 52, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 24 },
  ...
  sectionLabel: { color: Trading.muted, fontSize: 10, fontWeight: '700', letterSpacing: 2, marginBottom: 8 },
  sectionBody: { backgroundColor: Trading.surface, borderRadius: 10, borderWidth: 1, borderColor: Trading.border, overflow: 'hidden' },
  chipRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, padding: 12 },
  chip: { paddingHorizontal: 14, paddingVertical: 9, borderRadius: 8, borderWidth: 1, borderColor: Trading.border },
  ...
  stepperRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 24, padding: 14 },
  stepBtn: { width: 44, height: 44, borderRadius: 10, alignItems: 'center', justifyContent: 'center', backgroundColor: Trading.surfaceAlt, borderWidth: 1, borderColor: Trading.border },
  ...
  stratRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 14, borderWidth: 1.5, borderColor: 'transparent', borderBottomWidth: 1, borderBottomColor: Trading.border },
  ...
  checkRow: { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 14, borderBottomWidth: 1, borderBottomColor: Trading.border },
  checkBox: { width: 22, height: 22, borderRadius: 6, borderWidth: 1.5, alignItems: 'center', justifyContent: 'center' },
  checkMark: { color: Trading.bg, fontSize: 13, fontWeight: '900' },
  checkLabel: { fontSize: 14, fontWeight: '700' },
  resetSection: { margin: 14, backgroundColor: Trading.surface, borderRadius: 10, borderWidth: 1, borderColor: Trading.border, padding: 16, gap: 8 },
  ...
  resetBtn: { height: 44, alignItems: 'center', justifyContent: 'center', backgroundColor: Trading.risky + '18', borderRadius: 8, borderWidth: 1, borderColor: Trading.risky + '60' },
